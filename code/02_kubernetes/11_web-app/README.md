# Συνθετικό παράδειγμα εφαρμογής ιστού με βάση δεδομένων

Το τελευταίο αυτό παράδειγμα λειτουργεί ως σύνθεση των εννοιών που προηγήθηκαν. Η εφαρμογή αποτελείται από βάση PostgreSQL, επίπεδο εξυπηρέτησης ιστού με PHP/Apache, `Secret` για τον κωδικό πρόσβασης, `ConfigMap` για τη μη ευαίσθητη παραμετροποίηση, `PersistentVolumeClaim` για τη μόνιμη αποθήκευση της βάσης και αντικείμενα `Service` για τη σταθερή δικτυακή πρόσβαση.
