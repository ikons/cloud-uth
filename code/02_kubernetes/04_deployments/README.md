...
- Να επιλέγετε `Deployment` (όχι σκέτο ReplicaSet) ως default για όλα τα stateless workloads.

## Σύνδεση με την ακολουθία

Το `Deployment` είναι ο **πρακτικός** τρόπος να δηλώσετε stateless εφαρμογές. Δημιουργεί και διαχειρίζεται ReplicaSets για εσάς, και είναι το αντικείμενο που θα χρησιμοποιήσετε στα επόμενα παραδείγματα (`09_stateless-app`, `10_autoscaling`, `12_app-from-source`). Στα επόμενα δύο βήματα θα δούμε πώς εξωτερικοποιούμε ρύθμιση (`ConfigMap`) και μυστικά (`Secret`) — τα δύο πιο συνηθισμένα δεδομένα που "μπαίνουν" σε ένα Deployment.
